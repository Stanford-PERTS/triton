...
sys.path.insert(0, '/usr/local/google-cloud-sdk/platform/google_appengine')  # local testing
...
# script's working directory, so any use of the filesystem was broken.
# Hack around this by restoring the cwd. Note that we can't just stop using
# this function because it allows tests to load SDK-provided python libs,
# like webapp2.
...
# of tests.
test_arg = sys.argv[1] if len(sys.argv) == 2 else None
...
if test_arg:
    # Expected input is '{suite name}.{test name}', but the module loader
    # expect the name of the class also. Convention is that the class name is
    # StandingCamel version of the file/module/suite name.
    if '.' in test_arg:
        suite_name, test_name = test_arg.split('.')
        class_name = string_util.separated_to_camel(suite_name, standing=True)
        module_path = '.'.join([
            'unit_testing', suite_name, class_name, test_name
        ])
    else:
        module_path = '.'.join(['unit_testing', test_arg])

    suite = unittest.loader.TestLoader().loadTestsFromName(module_path)
else:
